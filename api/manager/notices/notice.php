<?php
// api/manager/notices/notice.php
...
function handleGetRequests($svc, $managerId, $action) {
    switch ($action) {
        case 'list':
            // GET /api/manager/notices/notice.php?action=list&status=active&priority=high&is_public=1&page=1&limit=10 (all optional)
            $result = $svc->getAllNotices();
            Response::json(['success' => true, 'data' => $result['notices'],]);
            break;
            
        case 'view':
            // GET /api/manager/notices/notice.php?action=view&id=1
