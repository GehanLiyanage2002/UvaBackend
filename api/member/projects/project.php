<?php
// api/member/projects/project.php
...
function handleGetRequests($svc, $memberId, $action) {
    switch ($action) {
        case 'list':
            // GET /api/member/projects/project.php?action=list
            $projects = $svc->listForMember($memberId);
            Response::json(['success' => true, 'tasks' => $projects]);
            break;
        case 'view':
            // GET /api/member/projects/project.php?action=view&id=1
            ...
            $task = $svc->getByIdWithDetailsByMember($memberId, $projectId);
